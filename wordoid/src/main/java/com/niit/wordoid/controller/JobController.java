...
import com.niit.wordoid.dao.JobApplicationDao;
import com.niit.wordoid.dao.JobDao;
...
public ResponseEntity<List<JobApplication>> getAppliedJobs(HttpSession session)
{
	...
	try
	{
		...
		return new ResponseEntity<List<JobApplication>>(jobApplicationDao.jobApplicationsByUserId(userDetails.getUserId()),HttpStatus.OK);
	}
	catch(NullPointerException e)
	{ 
		e.printStackTrace(); 
